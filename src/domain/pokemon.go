...
func GetPokemon(pokemonNumber int) (pokemon Pokemon, err error) {
	cmd := "SELECT name, type1, type2, height, weight, base_experience, image_url FROM pokemons WHERE pokemon_number = ? LIMIT 1"
	rows, err := Db.Query(cmd, pokemonNumber)
	log.Println(rows)
	if err != nil {
		log.Fatalln(err)
		return Pokemon{}, err
